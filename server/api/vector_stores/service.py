...
from api.v1.embeddings.handler import worker
from api.v1.vector_stores.files.repository import SimilaritySearchResult
import logging
...
class SearchEngine(BaseModel):
    engine_url: tp.Literal["https://duckduckgo.com", "https://bing.com"] = Field(
    ...
    async def run(self, *, q: str | None = None, url: str | None = None):
        if not q and not url:
            raise ValueError("No input parameter")
        browser = await launch(headless=True, args=["--no-sandbox"])
        page = await browser.newPage()
        try:
            if q:

                async for webpage in self.get_content(url=page.url, browser=browser):
                    visited = self.check_visited(url_=webpage.url)
                    if isinstance(visited, PageContent):
                        return visited
                    elif visited:
                        await self.run(url=webpage.url)
            elif url:
                if self.check_visited(url_=url):
                    page = await browser.newPage()
                    await page.goto(url)  # type: ignore
                    async for webpage in self.get_content(url=url, browser=browser):
                        visited = self.check_visited(url_=webpage.url)
                        if isinstance(visited, PageContent):
                            return visited
                        elif visited:
    ...
    async def create_engine(self, *, browser: browser.Browser):
        page = await browser.newPage()
        await page.setUserAgent(USER_AGENT)
        await page.goto(self.engine_url)  # type: ignore
        await page.waitForSelector(TEXT_AREA_SELECTOR)  # type: ignore
        await page.type(TEXT_AREA_SELECTOR, q)  # type: ignore
        await page.waitForSelector(BUTTON_SELECTOR)  # type: ignore
        await page.click(BUTTON_SELECTOR)  # type: ignore
        await page.waitForNavigation()  # type: ignore and

    async def get_content(self, *, url: str, browser: browser.Browser):
        page = await browser.newPage()
        await page.goto(url)  # type: ignore
        ...
        for link in data:
            ...
            item = PageContent(url=href, content=content)
            yield item

    def check_visited(self, *, url_: str):
        if url_ not in self.urls and url_.startswith("https://"):
            self.urls.append(url_)
            if len(self.urls) >= self.n:
                return self.pages
            logger.info("Visited url %s", url_)
            return True
        return False
...
class FileSearchTool(DocumentObject, Generic[T]):
    ...
    async def search(self, *, query: str, vector_store_id: str, top_k: int):
        ...
        documents = [f for f in FileObjectDocumentChunk.scan(store_id=vector_store_id)]
