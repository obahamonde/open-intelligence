...
class SpeechResource(Resource[Session, StreamingResponse, OpenAISpeechRequest]):
	def __load__(self):
		return Session(region_name="us-east-1")

	async def fetch(self, *, input: OpenAISpeechRequest):
...
async def text_to_speech(request: OpenAISpeechRequest):
